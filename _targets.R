...
initial_targets <- tar_plan(
  tar_target(temp, generate_template() ),
  tar_target(temp_csv, readr::write_csv(temp, here::here("Intermediate/Template.csv"))),
  tar_target(virus.test,c("Adeno-associated virus - 3", 
  "Adeno-associated virus 3B",
  "Adenovirus predict_adv-20",
  "A bad name")),
  tar_target(host.test, c("Equus caballus ferus",
    "Homo sapiens",
    "Hongus bongus",
    "Chiroptera",
    "Mus",
...
clover_targets <- tar_plan(
  ...
  tar_target(clo_formatted, format_clover(clo,temp)),
...
genbank_digest_targets <- tar_plan(
    tar_target(gb, tibble::as_tibble(seq)),
    ...
    tar_target(host_table_path, readr::write_csv(host_table, here::here("./Intermediate/GBHostTax.csv")),
